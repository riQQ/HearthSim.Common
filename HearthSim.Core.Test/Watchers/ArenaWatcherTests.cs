...
namespace HearthSim.Core.Test.Watchers
{
	...
	public class ArenaWatcherTests
	{
		...
		private MockArenaData _provider;
		private ArenaWatcher _watcher;

		public class MockArenaData : MockGameData
		{
			...
			public Card[] DraftChoices
			{
				get => _draftChoices;
				set => _draftChoices = value;
			}
		}
		...
		{
			_watcher.ChoicesChanged += args =>
			{
				_currentChoices = args.Choices;
				_choicesChangedCalls++;
			};
			_watcher.CardPicked += args =>
			{
				_currentPick = args.Picked;
				_cardPickedCalls++;
			};
			_watcher.DeckComplete += args =>
			{
				_currentArenaInfo = args.Info;
				_completeDeckCalls++;
			};
			_watcher.RunComplete += args =>
			{
		...
		public void FinalPick_SingleUpdate()
		{
			...
			_watcher.Update();
			_provider.DraftChoices = null;
			var updateResult = _watcher.Update();
			Assert.AreEqual(Watcher.UpdateResult.Break, updateResult);
