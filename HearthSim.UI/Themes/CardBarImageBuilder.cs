...
namespace HearthSim.UI.Themes
{
	public abstract class CardBarImageBuilder
	{
		...
		protected Typeface NumbersTypeFace = new Typeface(new FontFamily(new Uri("pack://application:,,,/HearthSim.UI;component/Resources/"), "./#Chunkfive"), FontStyles.Normal,
		...
		protected CardBarImageBuilder(ThemeConfig config, CardViewModel card, string dir)
		{
			_config = config;
			Card = card;
			ThemeDir = dir;
		...
		protected virtual void AddFrame()
		{
			...
			if(_config.RarityCardFrames && HasAllOptionalFrames)
			{
				switch(Card.Rarity)
				{
					case Rarity.RARE:
		...
		protected virtual void AddGem()
		{
			...
			if(_config.RarityCardGems && HasAllOptionalGems)
			{
				switch(Card.Rarity)
				{
					case Rarity.RARE:
		...
		protected virtual void AddCountBox()
		{
			...
			if(_config.RarityCardFrames && HasAllOptionalCountBoxes)
			{
				switch(Card.Rarity)
				{
					case Rarity.RARE:
		...
		protected void AddCost(Rect rect) => AddText(Card.Cost, CostFontSize, rect, Card.TextColor, NumbersTypeFace, 3.0, true);
		...
		protected void AddCardName(Rect rect)
			=> AddText(Card.Name, TextFontSize, rect, Card.TextColor, TextTypeFace);
