...
namespace HearthSim.Core.LogConfig
{
	internal class LogConfigItem
	{
		...
		{
			Name = name;
			if(verbose || LogConfigConstants.Verbose.Contains(name))
			{
		...
		public bool VerifyAndUpdate()
		{
			...
			foreach(var field in GetType().GetFields())
			{
				...
				Log.Debug($"[{Name}] set {field.Name}={req}");
				modified = true;
			}
			return modified;
		}
		...
		public override string ToString() => GetType().GetFields().OrderBy(x => x.MetadataToken)
			.Aggregate($"[{Name}]" + Environment.NewLine,
				(c, prop) => c + $"{prop.Name}={prop.GetValue(this)}" + Environment.NewLine);
	}
}
